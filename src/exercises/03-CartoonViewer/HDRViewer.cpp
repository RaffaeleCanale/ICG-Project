...
init()
{
	...
	m_mainShadingTexture.create(4, 1, GL_RGB, GL_RGB, GL_FLOAT, tex, GL_NEAREST);
}
...
reshape(int _w, int _h)
{
	...
	m_mainOutputTexture.create(_w,_h,GL_RGB,GL_RGB,GL_UNSIGNED_BYTE);
	...
	m_fbo.attachTexture(GL_COLOR_ATTACHMENT0_EXT, m_mainOutputTexture.getID());
...
drawScene() {
	...
	// set parameters
	m_mainShader.setMatrix4x4Uniform("worldcamera", m_camera.getTransformation().Inverse());
	m_mainShader.setMatrix4x4Uniform("projection", m_camera.getProjectionMatrix());
	m_mainShader.setMatrix4x4Uniform("modelworld", m_mesh.getTransformation() );
	...
	m_mainShadingTexture.setLayer(0);
	m_mainShadingTexture.bind();
	m_mainShader.setIntUniform("texture", m_mainShadingTexture.getLayer());
